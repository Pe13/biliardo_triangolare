...
class Biliardo {
  ...
  void addParticleToMultipleLaunch(std::vector<Particle>& launch);
  [[nodiscard]] bool findNextCollision(CollisionParameters& parameters) const;
  void launchForHistograms(Particle& particle) const;
   ...
   */
  void launchForDrawing_(double initialY, double initialDirection,
 ...
 public:
   ...
   * @param initialDirection Direzione iniziale della particella (compresa tra [-PI/2; PI/2])
   * @param output Il vettore che viene riempito con il risultato del lancio
   * @param shouldCheck Se viene passato true viene eseguito un check sulla validità dei parametri
  ...
  /**
   * @brief Lancia una singola particella fornendo solo la direzione iniziale e generando l'altezza
   * iniziale secondo una distribuzione uniforme tra tutti i valori accettati.
   *
   * @param initialDirection Direzione iniziale della particella (compresa tra [-PI/2; PI/2])
   * @param output Il vettore che viene riempito con il risultato del lancio
   * @param shouldCheck Se viene passato true viene eseguito un check sulla validità del parametro
   * initialDirection. Se non è necessario che il metodo esegua questo controllo il parametro può
   * essere omesso
   *
  ...
  bool launchForDrawingNoY(double initialDirection, std::vector<double>& output,
                           bool shouldCheck = true);

  /**
   * @brief Lancia una singola particella fornendo solo l'altezza iniziale e generando la direzione
   * iniziale secondo una distribuzione uniforme tra tutti i valori accettati.
   *
   * @param initialY Altezza iniziale della particella (compresa tra [-r1; r1])
   * @param output Il vettore che viene riempito con il risultato del lancio
   * @param shouldCheck Se viene passato true viene eseguito un check sulla validità del parametro
   * initialY. Se non è necessario che il metodo esegua questo controllo il parametro può essere
   * omesso
   *
   * Lancia una sola particella e riempie un vettore con le posizioni di tutti gli urti tra essa e i
   * bordi del biliardo. Le ultime due posizioni sono occupate dalle direzioni finale e iniziale.
