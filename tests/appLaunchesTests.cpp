  ...
  TEST_CASE("Single launches") {
    SUBCASE("Valid parameters") {
      for (unsigned int i = 0; i < 3; i++) {
        ...
        CHECK(app.getSingleLaunches()[static_cast<bt::PoolType>(i)].size() == 2);
        CHECK(app.getSingleLaunchesIndexes()[static_cast<bt::PoolType>(i)] == 1);

        launches[1] = app.singleLaunch(1);
        for (unsigned int j = 0; j < 2; j++) {
          CHECK_FALSE(launches[j].expired());
        }
        CHECK(app.getSingleLaunches()[static_cast<bt::PoolType>(i)].size() == 3);
        CHECK(app.getSingleLaunchesIndexes()[static_cast<bt::PoolType>(i)] == 2);

        launches[2] = app.singleLaunch(std::nullopt, 0);
        for (unsigned int j = 0; j < 3; j++) {
          CHECK_FALSE(launches[j].expired());
        }
        CHECK(app.getSingleLaunches()[static_cast<bt::PoolType>(i)].size() == 4);
        CHECK(app.getSingleLaunchesIndexes()[static_cast<bt::PoolType>(i)] == 3);

        launches[3] = app.singleLaunch(-1.5, M_PI / 4);
        for (unsigned int j = 0; j < 4; j++) {
          CHECK_FALSE(launches[j].expired());
        }
        CHECK(app.getSingleLaunches()[static_cast<bt::PoolType>(i)].size() == 5);
        CHECK(app.getSingleLaunchesIndexes()[static_cast<bt::PoolType>(i)] == 4);

        // Controllo che i puntatori decadano quando i lanci vengono distrutti
        REQUIRE(app.modifyPool(1, 1, 1) == true);
  ...
  TEST_CASE("Multiple launches") {
    SUBCASE("Valid parameters") {
      for (unsigned int i = 0; i < 3; i++) {
        ...
        const auto validLaunch1 = app.multipleLaunch(1e6, 0., app.pool().r1() / 5, 0, M_PI / 8);
        CHECK_FALSE(validLaunch1.expired());
        CHECK(app.getMultipleLaunches()[static_cast<bt::PoolType>(i)].size() == 1);
        CHECK(app.getMultipleLaunchesIndexes()[static_cast<bt::PoolType>(i)] == 0);

        const auto validLaunch2 = app.multipleLaunch(1e6, 0., app.pool().r1() / 7, 0, M_PI / 9);
        CHECK_FALSE(validLaunch2.expired());
        CHECK_FALSE(validLaunch1.expired());
        CHECK(app.getMultipleLaunches()[static_cast<bt::PoolType>(i)].size() == 2);
        CHECK(app.getMultipleLaunchesIndexes()[static_cast<bt::PoolType>(i)] == 1);
    ...
    SUBCASE("Invalid parameters") {
      ...
      CHECK(app.multipleLaunch(1e6, 0., -app.pool().r1() / 5, 0, M_PI / 8).expired());
      ...
      CHECK(app.multipleLaunch(1e6, 0., app.pool().r1() / 5, 0, 0).expired());
      CHECK(app.getMultipleLaunches() == multipleLaunches);
      CHECK(app.getMultipleLaunchesIndexes() == multipleLaunchesIndexes);
